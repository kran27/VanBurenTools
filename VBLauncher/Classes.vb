...
Public Class Map
    ...
    Property EMFG As EMFGc
    Property EMNO As New List(Of EMNOc)
    ...
    Function ToByte() As IEnumerable(Of Byte)
        ...
        If EMFG IsNot Nothing Then b.AddRange(EMFG.ToByte())
        b.AddRange(EMNO.SelectMany(Function(x) x.ToByte()))
...
Public Class USE ' also encapsulates CON and DOR, which use static chunks that are not read from or written to, if/which chunk is used is controlled by GOBJ.
    ...
    Function ToByte() As IEnumerable(Of Byte)
        ...
        If GOBJ.Type = 0 Then
        ElseIf GOBJ.Type = 1 Then
            b.AddRange(New Byte() {&H47, &H44, &H4F, &H52, &H1, &H0, &H0, &H0, &H18, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0}) ' GDOR chunk
        ElseIf GOBJ.Type = 2 Then
            b.AddRange(New Byte() {&H47, &H43, &H4F, &H4E, &H1, &H0, &H0, &H0, &H1A, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0}) ' GCON chunk
        End If
        Return b
    End Function
...
Public Class EMEFc
    ...
    Function ToByte() As IEnumerable(Of Byte)
        ...
        out.Write(41 + s1.Length + s2.Length, b)
        Return out
    End Function
...
Public Class EMNOc
    Property l As Point2 ' technically XYZ Coordinates
    ...
    Function ToByte() As IEnumerable(Of Byte)
        Dim out = New Byte(29 + tex.Length) {}
        out.Write(0, "EMNO")
        out.Write(8, 30 + tex.Length)
        out.Write(12, l.x)
        out.Write(20, l.y)
        out.Write(24, tex.Length)
        out.Write(26, tex)
        out.Write(26 + tex.Length, sr)
        Return out
    End Function
...
Public Class EMFGc
    ...
    Property base_height As Single
    Property anim1Speed As Single
    Property anim1Height As Single
    Property total_height As Single
    Property anim2Speed As Single
    Property anim2Height As Single
    Property verticalOffset As Single
    Property max_fog_density As Single
    ...
    Function ToByte() As IEnumerable(Of Byte)
        ...
        End If
        If base_height > total_height Then
            Throw New Exception("base_height > total_height!")
        End If
        If total_height <= 0 Then
            Throw New Exception("total_height <= 0.0f!")
        End If
        If max_fog_density <= 0 Or max_fog_density > 1 Then
            Throw New Exception("invalid max_fog_density")
        End If
        Dim out = New Byte(71) {}
        out.Write(0, "EMFG")
        out.Write(8, 72)
        out.Write(12, enabled)
        out.Write(13, colour.ToByte())
        out.Write(16, base_height)
        out.Write(20, anim1Speed)
        out.Write(24, anim1Height)
        out.Write(28, total_height)
        out.Write(32, anim2Speed)
        out.Write(36, anim2Height)
        out.Write(40, verticalOffset)
        out.Write(44, max_fog_density)
        Return out
    End Function
...
Public Class EMNPc
    Property chunks As List(Of EMNPChunk)

    Sub New()
        chunks = New List(Of EMNPChunk)
    End Sub

    Function ToByte() As IEnumerable(Of Byte)
        Dim out = New Byte(15 + (chunks.Count * 30)) {}
        out.Write(0, "EMNP")
        out.Write(8, 16 + (chunks.Count * 30))
        out.Write(12, chunks.Count)
        For i = 0 To chunks.Count - 1
            out.Write(16 + (i * 30), chunks(i).ToByte())
        Next
    End Function

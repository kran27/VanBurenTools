...
namespace VBLauncher
{
    ...
    public static class Extensions
    {
        ...
        {
            string @type = GetString(b.Skip(1).Take(1));
            return new ExTRc()
            {
                @type = @type,
                s = Conversions.ToString(type == "B" ? b[12] : global::VBLauncher.Extensions.GetString(b.Skip(14).Take(b[12])))
            };
        }

        public static ECAMc ToECAMc(this byte[] b)
        {
            return new ECAMc() { p = new Point4(BitConverter.ToSingle(b, 12), BitConverter.ToSingle(b, 16), BitConverter.ToSingle(b, 20), BitConverter.ToSingle(b, 24)) };
        }
        ...
        {
            int cl = b.Locate(Encoding.ASCII.GetBytes("EEOV"))[0];
            return new EME2c()
            {
                name = GetString(b, 14, b[12]),
                l = new Point4(BitConverter.ToSingle(b, 14 + b[12]), BitConverter.ToSingle(b, 18 + b[12]), BitConverter.ToSingle(b, 22 + b[12]), BitConverter.ToSingle(b, 26 + b[12])),
        ...
        public static EEN2c ToEEN2c(this byte[] b)
        {
            ...
            return new EEN2c()
            {
                ...
                sel = Conversions.ToBoolean(b[s3o + s3l + 1]),
                EEOV = b.Skip(cl).Take(BitConverter.ToInt32(b, cl + 8)).ToArray().ToEEOVc()
            };
        }
        ...
        public static GCREc ToGCREc(this byte[] b)
        {
            ...
            int sl = b[72] * 8; // Skills added length
            int cl = b[76 + sl] * 8; // Characters added length
            int tl = b[80 + sl + cl] * 4; // Traits added length
            int tsl = b[84 + sl + cl + tl] * 4; // Tag Skills added length
            int po = 94 + sl + cl + tl + tsl; // Portrait String offset
            byte pl = b[92 + sl + cl + tl + tsl]; // Portrait String Length
            int Heamo = 131 + sl + cl + tl + tsl + pl;
            int Heato = Heamo + 2 + b[Heamo - 2];
            int Haimo = Heato + 2 + b[Heato - 2];
            int Haito = Haimo + 2 + b[Haimo - 2];
            int Ponmo = Haito + 2 + b[Haito - 2];
            int Ponto = Ponmo + 2 + b[Ponmo - 2];
            int Musmo = Ponto + 2 + b[Ponto - 2];
            int Musto = Musmo + 2 + b[Musmo - 2];
            int Beamo = Musto + 2 + b[Musto - 2];
            int Beato = Beamo + 2 + b[Beamo - 2];
            int Eyemo = Beato + 2 + b[Beato - 2];
            int Eyeto = Eyemo + 2 + b[Eyemo - 2];
            int Bodmo = Eyeto + 2 + b[Eyeto - 2];
            int Bodto = Bodmo + 2 + b[Bodmo - 2];
            int Hanmo = Bodto + 2 + b[Bodto - 2];
            int Hanto = Hanmo + 2 + b[Hanmo - 2];
            int Feemo = Hanto + 2 + b[Hanto - 2];
            int Feeto = Feemo + 2 + b[Feemo - 2];
            int Bacmo = Feeto + 2 + b[Feeto - 2];
            int Bacto = Bacmo + 2 + b[Bacmo - 2];
            int Shomo = Bacto + 2 + b[Bacto - 2];
            int Shoto = Shomo + 2 + b[Shomo - 2];
            int Vanmo = Shoto + 2 + b[Shoto - 2];
            int Vanto = Vanmo + 2 + b[Vanmo - 2];
            int psl = sl + cl + tl + tsl + pl + b[Heamo - 2] + b[Heato - 2] + b[Haimo - 2] + b[Haito - 2] + b[Ponmo - 2] + b[Ponto - 2] + b[Musmo - 2] + b[Musto - 2] + b[Beamo - 2] + b[Beato - 2] + b[Eyemo - 2] + b[Eyeto - 2] + b[Bodmo - 2] + b[Bodto - 2] + b[Hanmo - 2] + b[Hanto - 2] + b[Feemo - 2] + b[Feeto - 2] + b[Bacmo - 2] + b[Bacto - 2] + b[Shomo - 2] + b[Shoto - 2] + b[Vanmo - 2] + b[Vanto - 2];

            #endregion

            #region Build Sections

            int[] gl = b.Locate(Encoding.ASCII.GetBytes("GWAM"));
            var tr = new List<int>();
            int io = 84 + cl + sl;
        ...
        {
            var p3 = new Point3(BitConverter.ToSingle(b, offset), BitConverter.ToSingle(b, offset + 4), BitConverter.ToSingle(b, offset + 8));
            string s = GetString(b, offset + 14, b[offset + 12]);
            var p2 = new Point2(BitConverter.ToSingle(b, offset + 14 + s.Length), BitConverter.ToSingle(b, offset + 18 + s.Length));
        ...
        public static GITMc ToGITMc(this byte[] b)
        {
            ...
            var Sho = b.ReadSocket(75 + a + Hea.Length + Eye.Length + Bod.Length + Bac.Length + Han.Length + Fee.Length);
            var Van = b.ReadSocket(79 + a + Hea.Length + Eye.Length + Bod.Length + Bac.Length + Han.Length + Fee.Length + Sho.Length);
            var IHS = b.ReadSocket(84 + a + Hea.Length + Eye.Length + Bod.Length + Bac.Length + Han.Length + Fee.Length + Sho.Length + Van.Length);
            return new GITMc()
            {
                @type = b[8 + a],
                equip = b[16 + a] != 0,
        ...
        {
            string model = GetString(b, i + 2, b[i]);
            string tex = GetString(b, i + 4 + b[i], b[i + 2 + b[i]]);
            return new Socket(model, tex);
        }

        public static EMNPChunk ReadEMNPChunk(this byte[] b, int offset)
        {
            bool @bool = b[offset] != 0;
            var p3 = new Point3(BitConverter.ToSingle(b, offset + 1), BitConverter.ToSingle(b, offset + 5), BitConverter.ToSingle(b, offset + 9));
            byte b1 = b[offset + 13];
            byte b2 = b[offset + 14];
            byte b3 = b[offset + 15];
            byte b4 = b[offset + 16];
        ...
        public static EMNOc ToEMNOc(this byte[] b)
        {
            ...
            string tex = GetString(b, 26, b[24]);
            int sr = BitConverter.ToInt32(b, 26 + tex.Length);
        ...
        public static EMFGc ToEMFGc(this byte[] b)
        {
            ...
            float base_height = BitConverter.ToSingle(b, 16);
            float anim1Speed = BitConverter.ToSingle(b, 20);
            float anim1Height = BitConverter.ToSingle(b, 24);
            float total_height = BitConverter.ToSingle(b, 28);
            float anim2Speed = BitConverter.ToSingle(b, 32);
            float anim2Height = BitConverter.ToSingle(b, 36);
            float verticalOffset = BitConverter.ToSingle(b, 40);
            float max_fog_density = BitConverter.ToSingle(b, 44);
        ...
        {
            int strStart = BitConverter.ToInt32(b, 12);
            int[] l = b.Locate(new byte[] { 0xD, 0xA });
            foreach (var m in l)
            {
                if (m >= strStart)
                {
        ...
        public static byte[] ToFixedBytes(ref IEnumerable<string> s)
        {
            ...
            var b = bl;
            int[] l = b.ToArray().Locate(new byte[] { 0x7C, 0x7E });
            foreach (var m in l)
        ...
        {
            var cf = new CRT();
            cf.EEN2 = ToEEN2c(b.GetRegions("EEN2")[0]);
            cf.GENT = ToGENTc(b.GetRegions("GENT")[0]);
        ...
        public static Map ReadMap(this byte[] b)
        {
            ...
            cf.EME2 = (from x in b.GetRegions("EME2")
                       select x.ToEME2c()).ToList();
            cf.EMEP = (from x in b.GetRegions("EMEP")
        ...
        private readonly static int[] Empty = new int[0];
    ...
    /// <returns>An integer array containing all locations of the given bytes</returns>
        ...
        {
            if (self is null | candidate is null | self.Count() == 0 | candidate.Count() == 0 | candidate.Count() > self.Count())
            {
                return Empty;
            }
...
if (match)
{
   list.Add(i);
            ...
            int[] sortedList = list.OrderBy(i => i).ToArray(); // ensure list is in order of location for (hopefully) identical file output
        ...
        {
            byte[] hn = Encoding.ASCII.GetBytes(hs);
            int[] hc = b.Locate(hn);
            return (from l in hc
        ...
        {
            int[] hc = b.Locate(Encoding.ASCII.GetBytes("EMTR"));
            return (from l in hc
                    let tl = BitConverter.ToInt32(b, l + 8)
    ...
    /// </summary>
        ...
        {
            Write(b, offset, new byte[] { (byte)(value ? 1 : 0) });
        }

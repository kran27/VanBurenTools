...
inline bool wireframe() {
  return ReadPtr<char>(HudBase, {0x2C, 0x74, 0x58}) == 1;
}
...
inline void OutlinedText(ImDrawList *dl, const ImFont *fnt, const float sz,
                         const ImVec2 pos, char const *fmt, ...) {
  char text[256];
  va_list args;
  va_start(args, fmt);

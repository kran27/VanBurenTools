...
#define setuptext                                                              \
  ...
  ImGui::SetCursorPosX(12); ImGui::Text(
#define setupcontrol );                                                        \
  ImGui::SameLine();                                                           \
  ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 3);                            \
...
typedef void(__stdcall *D3D11DrawIndexedInstancedIndirectHook)(
    ID3D11DeviceContext *pContext, ID3D11Buffer *pBufferForArgs,
    UINT AlignedByteOffsetForArgs);
D3D11DrawIndexedInstancedIndirectHook phookD3D11DrawIndexedInstancedIndirect =
    NULL;
...
typedef void(__stdcall *D3D11DrawHook)(ID3D11DeviceContext *pContext,
                                       UINT VertexCount,
                                       UINT StartVertexLocation);
D3D11DrawHook phookD3D11Draw = NULL;

typedef void(__stdcall *D3D11DrawIndexedHook)(ID3D11DeviceContext *pContext,
                                              UINT IndexCount,
...
bool get_present_pointer() {
  ...
  if (D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, 0,
    ...
    phookD3D11DrawIndexedInstancedIndirect =
        (D3D11DrawIndexedInstancedIndirectHook)p_vtable[39];
    phookD3D11DrawIndexedInstanced =
        (D3D11DrawIndexedInstancedHook)p_vtable[20];
...
public:
  Model(UINT Stride, UINT vscWidth, UINT pscWidth, UINT psDescrFORMAT) {
    ModelStride = Stride;
    ModelvscWidth = vscWidth;
    ModelpscWidth = pscWidth;
    ModelpsDescrFORMAT = psDescrFORMAT;
  };

  bool IsModel(UINT Stride, UINT vscWidth, UINT pscWidth) {
    if (Stride == ModelStride && vscWidth == ModelvscWidth &&
        pscWidth == ModelpscWidth)
      return true;
    else
      return false;
  };
  bool IsModel(UINT Stride, UINT vscWidth, UINT pscWidth, UINT psDescrFORMAT) {
    if (Stride == ModelStride && vscWidth == ModelvscWidth &&
        pscWidth == ModelpscWidth && psDescrFORMAT == ModelpsDescrFORMAT)
      return true;
    else
      return false;
  };
...
static long __stdcall detour_present(IDXGISwapChain *p_swap_chain,
                                     UINT sync_interval, UINT flags) {
  if (!imguiinit) {
    if (SUCCEEDED(p_swap_chain->GetDevice(__uuidof(ID3D11Device),
                                          (void **)&p_device))) {
      ...
      p_swap_chain->GetBuffer(0, __uuidof(ID3D11Texture2D),
                              (LPVOID *)&pBackBuffer);
      p_device->CreateRenderTargetView(pBackBuffer, NULL,
      ...
      D3DX11CreateShaderResourceViewFromMemory(p_device, attack_cursor,
                                               sizeof(attack_cursor), &info,
                                               pump, &attackCursor, nullptr);
      D3DX11CreateShaderResourceViewFromMemory(p_device, default_cursor,
                                               sizeof(default_cursor), &info,
                                               pump, &defaultCursor, nullptr);
      D3DX11CreateShaderResourceViewFromMemory(p_device, gui_cursor,
                                               sizeof(gui_cursor), &info, pump,
                                               &guiCursor, nullptr);
      D3DX11CreateShaderResourceViewFromMemory(p_device, pickup_cursor,
                                               sizeof(pickup_cursor), &info,
                                               pump, &pickupCursor, nullptr);
      D3DX11CreateShaderResourceViewFromMemory(p_device, target_cursor,
                                               sizeof(target_cursor), &info,
                                               pump, &targetCursor, nullptr);
      D3DX11CreateShaderResourceViewFromMemory(p_device, text_cursor,
                                               sizeof(text_cursor), &info, pump,
                                               &textCursor, nullptr);
      ...
      ImFont *font = io.Fonts->AddFontFromFileTTF(
          R"(C:\Windows\Fonts\segoeui.ttf)", 17.0f);
      hrFont = io.Fonts->AddFontFromFileTTF(R"(C:\Windows\Fonts\segoeui.ttf)",
                                            20.0f);
...
#pragma endregion
  ...
  if (g_ResizeWidth != 0 && g_ResizeHeight != 0) {
    ...
    p_swap_chain->ResizeBuffers(0, g_ResizeWidth, g_ResizeHeight,
  ...
  if (show) {
    ...
    if (ImGui::TabItemButton("Info"))
      tab = 1;
    if (ImGui::TabItemButton("Utilities"))
      tab = 2;
    if (ImGui::TabItemButton("Toggles"))
      tab = 3;
    if (ImGui::TabItemButton("Extender Options"))
      tab = 4;
    if (ImGui::TabItemButton("Model Finder"))
      tab = 5;
    ...
    case 1: {
      ...
      ImGui::Text("Cursor Position: %.2f, %.2f, %.2f", cursorPos[0],
                  cursorPos[1], cursorPos[2]);
      auto ent = GetAddr(EntityBase, {0xE8, (uint)selEntity * 4, 0x2C, 0x0});
      auto gent = (int)RTDynamicCast((uint32 *)ent, 0, Entity, GameEntity, 0);
      setuptext "Entity" setupcontrol ImGui::Combo("", &selEntity, entities,
                                                   IM_ARRAYSIZE(entities));
      setuptext "Position" setupcontrol ImGui::InputFloat3(
          "\u200B", entPos, "%.2f", ImGuiInputTextFlags_ReadOnly);
      setuptext "Health" setupcontrol if (ImGui::SliderInt("\u200B\u200B", &HP,
                                                           0, 999)) {
        Write<int>(gent + 0x24, HP);
      }
      setuptext "Action Points" setupcontrol if (ImGui::SliderInt(
                                                     "\u200B\u200B\u200B", &AP,
                                                     0, 100))
          WritePtr<int>(EntityBase, {0xE8, (uint)selEntity * 4, 0x2C, 0x174},
                        AP);

      ImGui::Text("Special: %d, %d, %d, %d, %d, %d, %d", Special[0], Special[1],
                  Special[2], Special[3], Special[4], Special[5], Special[6]);
      ...
      auto entPtr = (const char *)GetAddr(
          EntityBase, {0xE8, (uint)selEntity * 4, 0x2C, 0x0});
      const char *name;     // eax
      ...
      ImGui::Text("Team ID and Rank: %d and %d", *((_DWORD *)entPtr + 0xDA),
                  *((_DWORD *)entPtr + 0xDB));
      ImGui::Text("Squad ID and Rank: %d and %d", *((_DWORD *)entPtr + 0xDC),
                  *((_DWORD *)entPtr + 0xDD));
    ...
    case 2: {
      setuptext
          "Game Speed" setupcontrol if (ImGui::SliderFloat("", &speed, 0.5f,
                                                           10.0f, "%.1f"))
              setAllToSpeed(speed);
      setuptext "Object X" setupcontrol ImGui::SliderFloat(
          "\u200B", &objX, 0.0f, 500.0f, "%.1f");
      setuptext "Object Y" setupcontrol ImGui::SliderFloat(
          "\u200B\u200B", &objY, 0.0f, 500.0f, "%.1f");
      setuptext "Object Z" setupcontrol ImGui::SliderFloat(
          "\u200B\u200B\u200B", &objZ, 0.0f, 500.0f, "%.1f");
      break;
    }
    case 3: {
      const auto sz = ImVec2(ImGui::GetWindowWidth() / 3 - 10.66f, 0.0f);
      if (ImGui::Button("Statistics", sz))
        ToggleStatistics();
      ImGui::SameLine();
      if (ImGui::Button("Lighting", sz))
        ToggleLighting();
      ImGui::SameLine();
      if (ImGui::Button("Shadow", sz))
        ToggleShadow();
      if (ImGui::Button("Camera Fog", sz))
        ToggleCameraFog();
      ImGui::SameLine();
      if (ImGui::Button("Wireframe", sz))
        ToggleWireframe();
      ImGui::SameLine();
      if (ImGui::Button("Transparency", sz))
        ToggleTransparency();
      if (ImGui::Button("Octree Drawing", sz))
        ToggleOctreeDrawing();
      ImGui::SameLine();
      if (ImGui::Button("Debug Light", sz))
        DebugLight();
      ImGui::SameLine();
      if (ImGui::Button("Fog of War", sz))
        ToggleFogOfWar();
      if (ImGui::Button("FOW Automap", sz))
        ToggleFOWAutomap();
      ImGui::SameLine();
      if (ImGui::Button("FOW LOS", sz))
        ToggleFOWLOS();
      ImGui::SameLine();
      if (ImGui::Button("Visual Effects", sz))
        ToggleVisualEffects();
      if (ImGui::Button("Water", sz))
        ToggleWater();
      ImGui::SameLine();
      if (ImGui::Button("Camera Constraints", sz))
        CameraToggleConstraints();
      ImGui::SameLine();
      if (ImGui::Button("Debug Window", sz))
        DebugWindow();
      if (ImGui::Button("Stack Window", sz))
        StackWindow();
      ImGui::SameLine();
      if (ImGui::Button("Debug Tools", sz))
        DebugTools();
      ImGui::SameLine();
      if (ImGui::Button("Scene", sz))
        ToggleScene();
      if (ImGui::Button("Entities", sz))
        ToggleEntities();
      ImGui::SameLine();
      if (ImGui::Button("GUI", sz))
        ToggleGUI();
      ImGui::SameLine();
      if (ImGui::Button("Debug Logs", sz))
        ToggleConsole();
      if (ImGui::Button("Rain", sz))
        ToggleRain();
      ImGui::SameLine();
      if (ImGui::Button("Snow", sz))
        ToggleSnow();
      ImGui::SameLine();
      if (ImGui::Button("Wind", sz))
        ToggleWind();
      break;
    }
    case 4: {
      ...
      ImGui::SameLine();
      HelpMarker(
          "Toggles use of the hand cursor when moving items in inventory");
      ImGui::Checkbox("Ignore Wireframe State", &ignoreWireframe);
      ImGui::SameLine();
      HelpMarker("Toggles whether the extender menus will follow the game's "
                 "wireframe state");
  ...
  fdl->AddImage(cursor, ImVec2(mp.x + offset.x, mp.y + offset.y),
                ImVec2(mp.x + size.x + offset.x, mp.y + size.y + offset.y),
...
skipcursor:
  ...
  if (showMouseOver || (altStats && renderStats)) {
    ...
    if (showMouseOver) {
      OutlinedText(bdl, io.FontDefault, io.FontDefault->FontSize,
                   ImVec2(mp.x, mp.y + 30), text);
    }

    if (altStats)
      OutlinedText(bdl, hrFont, 20.0f, ImVec2(2, 18), "Selected: %s", text);
  }
  ...
  if (renderStats) {
    if (altStats) {
      ...
      if (isValid(Map))
        OutlinedText(bdl, hrFont, 20.0f, ImVec2(2, -2), "Map: %s", Map);
      OutlinedText(bdl, hrFont, 20.0f, ImVec2(2, 38), "FPS: %.1f (%.1f ms)",
                   FPS, 1.0f / FPS * 1000);
      OutlinedText(bdl, hrFont, 20.0f, ImVec2(2, 58),
                   "Cam: (%.1f, %.1f, %.1f) %.1f FOV", CamX, CamZ, CamY, FOVy);
    } else {
      const auto Map = mapName();
      const auto FPS = Read<float>(SettingsBase + 0x6C4);
      ...
      if (isValid(Map))
        OutlinedText(bdl, hrFont, 20.0f, ImVec2(2, -2), "Map: %s", Map);
      OutlinedText(bdl, hrFont, 20.0f, ImVec2(2, 18),
                   "FPS: %.1f V: %d T: %d Vis: %d", FPS, V, T, Vis);
      OutlinedText(bdl, hrFont, 20.0f, ImVec2(2, 38),
                   "Tex: %d VB: %d Mtx: %d DIP: %d DP: %d Trans: %s Light: %s",
                   Tex, VB, Mtx, DIP, DP, Trans ? "On" : "Off",
                   Light ? "On" : "Off");
      OutlinedText(bdl, hrFont, 20.0f, ImVec2(2, 58), "Cam: (%.1f, %.1f, %.1f)",
                   CamX, CamZ, CamY);
      OutlinedText(bdl, hrFont, 20.0f, ImVec2(2, 78),
                   "Target: (%.1f, %.1f, %.1f)", TargetX, TargetZ, TargetY);
      OutlinedText(bdl, hrFont, 20.0f, ImVec2(2, 98),
                   "Azi: %.1f Elev: %.1f Dist: %.1f FOVy: %.1f", Azi, Elev,
                   Dist, FOVy);
    }
  }
  ...
  bdl->AddImage(targetCursor, ImVec2(screenPos[0] - 14, screenPos[1] - 14),
                ImVec2(screenPos[0] + 15, screenPos[1] + 15), ImVec2(0, 0),

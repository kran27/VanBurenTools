...
auto RegisterConsoleCommand = reinterpret_cast<RegisterConsoleCommandFunc>(F3 + 0x0019F5D0);
...
inline auto getCurrentEntityPtr() {
	...
	if (curEntity) return GetEntityPtr(curEntity);
}

inline auto getCurrentEntityID() {
	auto client = getClient();
	auto curEntity = client[0x37];
	if (curEntity) return curEntity;
}
...
{
	const auto ptr = reinterpret_cast<char*>(GetAddr(SettingsBase, { 0x0 }));
	const auto v2 = (float*)(*(int(__thiscall**)(char*))(*(_DWORD*)ptr + 0xAC))(ptr);
	const auto v11 = (float*)RTDynamicCast((_DWORD*)v2, NULL, Gfx_CamCtrl, Gfx_CamCtrl_Orbit, NULL);
	return v11;
}
...
inline auto EntityInfo()
{
	...
	DebugAndConsole("Team ID and Rank: %d and %d", *((_DWORD*)entPtr + 0xDA), *((_DWORD*)entPtr + 0xDB));
	DebugAndConsole("Squad ID and Rank: %d and %d", *((_DWORD*)entPtr + 0xDC), *((_DWORD*)entPtr + 0xDD));
